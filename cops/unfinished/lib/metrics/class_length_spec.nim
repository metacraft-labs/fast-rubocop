...
RSpec.describe(ClassLength, "config", proc () =
  var cop = ()
...
""".stripIndent)
    var offense = self.cop().offenses.first()
...
""".stripIndent)
  context("when a class has inner classes", proc () =
...
""".stripIndent))
  context("when inspecting a class defined with Class.new", proc () =
